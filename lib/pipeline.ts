 ...
 *
 * The audit pipelines repeat the same cross-cutting skeleton: validate → build
 * flags → route to a terminal status → credit hours → publish a public-cluster
 * row. The DOMAIN logic stays in each pipeline (food-audit per-item vision,
 * gov-audit browser checks); this module holds only the pieces that are
 * genuinely identical so they live in one place and can be unit-tested.
 *
 * What lives here:
 *  - `PendingFlag` — the single flag shape used by the food-audit pipeline.
 *  - `routePendingFlags` — the block/review → terminal-status decision (block ⇒
 *    rejected, any open flag ⇒ flagged, else verified). The generic-submission
 *    routing (`routeStatus` in lib/fraud.ts) is a different state machine and
 *    stays where it is.
