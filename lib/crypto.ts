 ...
 * encrypting OR decrypting with no key configured throws (fail-closed) so PII is
 * never written in cleartext and an `enc:v1:` ciphertext token is never surfaced
 * in place of decrypted PII. Plaintext passthrough is permitted ONLY under an
 * explicit DEMO_MODE flag or an explicit unit-test run (which runs with no key
 * by design) — never inferred from a missing Cloudflare context. `decryptField`
...
export async function decryptField(value: string | null | undefined): Promise<string | null> {
  ...
  if (!key) {
    ...
    // rather than leak the enc:v1: ciphertext token into the CF 888 / UI.
