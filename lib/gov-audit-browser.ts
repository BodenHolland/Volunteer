 ...
 *   - "skipped": produced when the SSRF gate refuses a URL — no readout at all.
 *
 * SSRF: a signed-in user supplies the URL the server loads in headless Chromium
 * (page.goto) and a fallback fetch, then returns a screenshot + title + status.
 * Every user-URL navigation/fetch is hard-gated by `checkSsrfUrl` BEFORE it runs
 * (https-only + gov allowlist + private/loopback/link-local IP block), every
 * redirect hop is re-validated, and the post-redirect landing URL is re-checked.
 *
 ...
 * session, so one session for N URLs beats N sessions. The caller should bound
 * the URL count (per-run anchor cap) before invoking.
 */
 ...
 * is an "Access Denied" page (see isBlockedPageTitle below); we surface a
