 ...
 *
 * Every credit site (org approval, food-audit pipeline, gov-audit approval,
 * seed) MUST go through here so the write is the SAME atomic upsert everywhere.
 * The race-prone read-modify-write pattern (SELECT total_hours → add → UPDATE)
 * is gone: two concurrent credits into the same (user, month, org) row can no
 * longer clobber each other, because the increment happens inside SQLite via
