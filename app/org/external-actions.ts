...
async function loadExternal(submissionId: string) {
  ...
  // their own work — these hours feed the CF 888 legal attestation. An org_member
  // can become a submission owner (the recipient task actions now also block that,
  // but defend here too), so reject self-review outright.
...
export async function approveExternalSubmission(formData: FormData) {
  ...
  let fileMeta: {
    ...
    project_name?: string;
    project_slug?: string;
    reported_cumulative_minutes?: number;
  ...
  // C1 (hard legal line): a Zooniverse certificate reports CUMULATIVE lifetime
  // minutes, so the creditable amount is only the DELTA above what this volunteer
  // has ALREADY been credited for this task — never the full self-reported figure,
  // which would re-credit the same lifetime hours every reporting month. (This was
  // previously enforced only on the env-gated auto-approve path; this human-review
  // path is the one prod actually uses, so it must enforce it too.) The reviewer
  // may credit LESS for quality; the delta — and any monthly cap — is a hard upper
  // bound. When the cert's cumulative is unknown, the delta is 0 (fail safe: never
  // credit an unbounded self-reported total).
  const priorCumulative = await priorCreditedCumulativeMinutes(sub.user_id, task.id);
  const reportedCumulativeMinutes = Math.max(
    0,
    Math.round(Number(fileMeta.reported_cumulative_minutes ?? Number(fileMeta.reported_hours ?? 0) * 60))
  );
  ...
  let clamped = Math.min(requestedMinutes, deltaMinutes);
  if (cap != null) {
  ...
  // Idempotency / single-flight (C5): the irreversible status='approved' flip is
  // compare-and-set (WHERE status != 'approved'). The ledger upsert is ADDITIVE,
  // so without this a double-click, replayed POST, or two concurrent approves of
  // the same submission would each add the credit again. D1 serializes writes, so
  // exactly ONE invocation flips the row; every other matches zero rows and bails
  // BEFORE crediting.
  const claim = await db
    .prepare(
      "UPDATE submissions SET status = 'approved', reviewed_at = ?, reviewer_id = ?, reviewer_notes = ?, hours_credited = ?, published_at = ? WHERE id = ? AND status != 'approved'"
    )
    .bind(now, user.id, note, creditedHours, now, id)
    .run();
  if (claim.meta.changes !== 1) {
    // Already approved (replay / lost race) — never credit twice.
    revalidatePath("/org/submissions");
    redirect("/org/submissions");
  }

  // Reviewer-visible warnings, only on the winning path so replays don't pile up
  // duplicate flag rows.
    ...
    await db
      ...
      .bind(
        ...
        JSON.stringify({
          ...
          granted: clamped,
          creditable_delta_minutes: deltaMinutes,
  ...
  // The review record + the ledger credit commit together (D1 implicit
  // transaction) behind the winning claim. certificate_reviews records
  // reported_cumulative_minutes (migration 0021) so the NEXT certificate's delta
  // (C1) diffs against this approval's cumulative baseline — without it the
  // baseline stays 0 and the delta protection silently fails open next month.
  await db.batch([
    db
      .prepare(
        `INSERT INTO certificate_reviews
         ...
         ON CONFLICT(submission_id) DO UPDATE SET
           ...
           reported_cumulative_minutes = excluded.reported_cumulative_minutes,
