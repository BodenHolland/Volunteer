...
export async function approveSubmission(formData: FormData) {
  ...
  // The irreversible status='approved' flip, the ledger credit, and the
  // public-cluster publish are grouped into one db.batch() so the approval can
  // never commit without its credited hours (D1 runs a batch as an implicit
  // transaction, rolling back fully on any failure).
