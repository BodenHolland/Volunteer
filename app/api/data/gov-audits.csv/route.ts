 ...
 *
 * DATA PRINCIPLE (CLAUDE.md): this endpoint reads the public cluster
 * (gov_audit_page_evaluations, gov_audit_site_evaluations, gov_audit_auto_checks).
 * The only private table touched is gov_audit_sessions, and ONLY its `status`
 * column, used as the publish gate (see below) — no PII column is ever selected.
 *
 * Publish gate: gov_audit_page_evaluations carries no status/published column,
 * so a row's publishability is determined by its session lifecycle. We inner-
 * JOIN gov_audit_sessions on public_session_ref filtered to status='finalized'.
 * Drafts / submitted / flagged sessions never export. (migrations 0013-0014.)
 *
 ...
 *
 * Streaming + keyset pagination: streamed page-by-page (keyset cursor on
