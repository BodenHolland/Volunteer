 ...
 *
 * Streaming + keyset pagination: the payload is streamed page-by-page so memory
 * stays bounded regardless of row count (no full-table read into an array).
 * Pages are walked by a keyset cursor on audit_item_captures.id (the PK), which
 * is stable under concurrent inserts — far cheaper than OFFSET on a large set.
 */
