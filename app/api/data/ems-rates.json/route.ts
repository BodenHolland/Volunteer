 ...
 *
 * M6 — Free-text gate: `notes` AND `zip_codes` (migration 0015 marks zip_codes
 * "free text — moderation status below gates publishing") publish only when
 ...
 * ems_rate_reports.id so memory stays bounded regardless of row count. The
 * previous shape ({ count, rows }) cannot be emitted up-front when streaming
 * (count is unknown until the walk completes), so the body is now a bare array.
 */
