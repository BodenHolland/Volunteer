# COLIFT — Operations Runbook

Operational reference for running COLIFT in production: backups, disaster
...
- `curl -I https://colift.xkbtrjm2bm.workers.dev` returns 200/redirect.
...
1. **Fast — roll back to the previous Worker version (no rebuild):**
   ...
   ```bash
   npx wrangler rollback --name colift            # rollback to the prior version
   npx wrangler rollback <version-id> --name colift -m "reason"   # to a specific version
   ```
...
- **Live logs (tail):** `npx wrangler tail colift --format=pretty` — streams Worker
...
- **Reseed / reset data** (wipes + reseeds ALL tables — destructive, demo/staging
  ...
  `curl -X POST https://colift.xkbtrjm2bm.workers.dev/api/admin/reset`. **Never run
