 ...
 * Because this runs under node:test, isTestRun() is true, so plaintextAllowed()
 * permits passthrough. These assertions verify that passthrough behavior holds.
 */
...
test("FAIL-CLOSED: no key + not a test run → encryptField throws (never writes cleartext)", async () => {
  ...
  try {
    ...
    // it must NOT surface the enc:v1: ciphertext token (the live H3 bug).
    await assert.rejects(
